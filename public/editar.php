...
$erros = [];
$dados = $prontuario; // pré-carrega com valores do BD
...
$breadcrumb = [
    ...
    ['label' => 'Prontuário #' . $id, 'url' => 'visualizar.php?id=' . $id],
    ['label' => 'Editar'],
];
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <h2 class="page-title mb-0">
        ...
        Editar Prontuário <?= h($prontuario['numero_prontuario'] ? '#' . $prontuario['numero_prontuario'] : '#' . $prontuario['id']) ?>
    </h2>
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <input type="text" class="form-control" id="numero_prontuario" name="numero_prontuario"
                    value="<?= h($dados['numero_prontuario'] ?? '') ?>">
            ...
            <div class="col-md-3">
                ...
                <select class="form-select" id="sexo" name="sexo">
                    ...
                    <?php foreach (['Masculino','Feminino','Outro'] as $op): ?>
                        <option value="<?= h($op) ?>" <?= ($dados['sexo'] ?? '') === $op ? 'selected' : '' ?>><?= h($op) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="col-md-3">
                ...
                <select class="form-select" id="estado_civil" name="estado_civil">
                    <option value="">Selecione...</option>
                    <?php foreach (['Solteiro(a)','Casado(a)','Divorciado(a)','Viúvo(a)','União Estável','Separado(a)'] as $ec): ?>
                        <option value="<?= h($ec) ?>" <?= ($dados['estado_civil'] ?? '') === $ec ? 'selected' : '' ?>><?= h($ec) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <select class="form-select" id="status" name="status">
                    <option value="digitado" <?= ($dados['status'] ?? '') === 'digitado' ? 'selected' : '' ?>>Digitado</option>
...
<div class="d-flex gap-2 justify-content-end mb-4">
    ...
    <button type="submit" class="btn btn-warning px-4">
        <i class="bi bi-floppy-fill me-2"></i>Atualizar Prontuário
    </button>
</div>
...
(function () {
    ...
    input.addEventListener('blur', function () {
        ...
        fetch('api/verificar-numero.php?numero=' + encodeURIComponent(numero) + '&exclude_id=' + excludeId)
            .then(r => r.json())
            .then(data => {
                if (data.existe) {
